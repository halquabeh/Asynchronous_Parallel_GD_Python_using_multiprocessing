...
Gradient Descent parallel with two cores
@author: halqu
"""
from numpy import *
from pandas import *
...
def GD(X_train,y_train,alpha,Ws,iterations,idd):
    ...
    for i in range(0,iterations):
        ...
        for i in range(n):
             Ws[i]=W[i,:]
        W = array(Ws);W=W.reshape((n,1))
    np.savez(str(idd)+'two1.npz', loss1=L, time1=T)
    return Ws
...
if __name__=='__main__':
    ...
    p1=mp.Process(target=GD,args=(X_train[:mid,:],y_train[:mid,:],alpha,W,iterations,1))
    p2=mp.Process(target=GD,args=(X_train[mid:,:],y_train[mid:,:],alpha,W,iterations,2))
    start_time = time.time()
    p1.start()
    p2.start()
    p1.join()
